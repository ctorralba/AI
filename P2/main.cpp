...
bool validAction(State& S, Action A){
  ...
  if (A.movObj == 'B'){
	...
	if (A.movement == 'n'){
		...
		cout << "C" << endl;
		if (S.boatY == S.gridWidth-1 && S.boatDir == 'L'){
			return false;
		}
		cout << "D" << endl;
		if (S.boatDir == 'U'){
		...
		cout << "G" << endl;
		for(int h =0 ; h < S.gridHeight; h++)
		{
		for(int w =0 ; w < S.gridWidth; w++)
		{
			cout << S.obsGrid[w][h] << ' ';
		}
		cout <<endl;
...
Node* childNode(Node* Parent, Action parentAction);
Node* childNode(Node* Parent, Action parentAction){
	Node* nodePtr = new Node;	
	nodeCreation++;
	nodePtr->parent = Parent;
	nodePtr->St = result(Parent->St, parentAction); //returns a new state.
	gridsCreated--;
...
*/
Node* BFTS(Node& root, LinkedList<Node*>& statesCreated);
Node* BFTS(Node& root, LinkedList<Node*>& statesCreated){
	...
	Node* rootPtr = &root;
	frontier.enqueue(rootPtr);
	statesCreated.insert_front(rootPtr);
	...
	for(int h =0 ; h < eval->St.gridHeight; h++)
	{
		for(int w =0 ; w < eval->St.gridWidth; w++)
		{
			cout << eval -> St.obsGrid[w][h] << ' ';
		}
		cout <<endl;
	}
	...
	while(!checkGoal(eval->St)){
		...
		while (possibleAction->m_next != NULL){
			...
			if (validAction(eval->St, possibleAction->m_data)){
			    ...
			    createdNode = childNode(eval, action); //parent's node, action
				frontier.enqueue(createdNode);
			
				statesCreated.insert_front(createdNode);
	...
	if (checkGoal(eval->St)){
		actList.clear();
		if (!frontier.isEmpty()){
			if (frontier.getFirst() < frontier.getLast()){
				for (int i = frontier.getFirst(); i < frontier.getLast(); i++){
					frontier.m_data[i] = NULL;
				}
			}
			else{// (frontier.getLast() < frontier.getFirst())
				for (int i = 0; i < frontier.getLast(); i++){
					frontier.m_data[i] = NULL;
				}
				for (int i = frontier.getFirst(); i < frontier.getMax(); i++){
					frontier.m_data[i] = NULL;
				}
			}
		}
...
/*void deleteNodes(LinkedList<Node*>& nodesDel, int gridW){
	LinkedList<Node*>* test = nodesDel.getFirstPtr();
	while (test->m_next != NULL) {
		cout << "testSize" << test->size() << endl;
		test->m_data->St.aList.clear();
    	test->m_data->St.tuList.clear();
    	test->m_data->St.trList.clear();
    	
    	for (int k = 0; k < gridW; k++){
    		delete [] test->m_data->St.radGrid[k];
		}
		delete [] test->m_data->St.radGrid;
		for (int k = 0; k < gridW; k++){
    		delete [] test->m_data->St.obsGrid[k];
		}
		delete [] test->m_data->St.obsGrid;
		delete test;
		test = test->m_next;
	}
	/*if (!nodesDel.isEmpty()){
		...
		int size = nodesDel.size();
		prevNode = nodesDel.getAtPtr(size-2);
		prevNode->m_next = NULL;
		prevNode=NULL;
	    test = nodesDel.getLastPtr();
		...
		delete [] test->m_data->St.obsGrid;
		cout << nodesDel.size() << endl;
    	delete test;
    	test=NULL;
    	deleteNodes(nodesDel, gridW);
	}
}*/

void clear(LinkedList<Node*> Frontier)
{
	LinkedList<Node*>* tmp = &Frontier;
...
int main()
{
	...
	clock_t begin = clock();
	Node* results = BFTS(root, frontierSpace);
	clock_t end = clock();
	
	double time_spent = double(end-begin)/CLOCKS_PER_SEC;
	time_spent = time_spent * 1000 * 1000; //conversion from sec to ms
    ...
    cout << "Nodes Created: " << nodeCreation << endl;
    cout << "Grids Created: " << gridsCreated << endl;
    
    
    /*Deallocation of nodes that were in frontier*/
    //call a function and deallocate recursively
    /*LinkedList<Node*>* remove = frontierSpace.getFirstPtr();
    while (remove->m_next != NULL){
    	remove->m_data->parent = NULL;
    	//cout << remove->m_data->St.boatDir << endl;
    	remove = remove->m_next;
	}
	
    deleteNodes(frontierSpace, gridW);*/
    solution.clear();
	return 0;
}
